...
<div class="head-top">
     ...
     <div id="sliderA" class="slider">
      @foreach($slice as $v)
      <a href="{{url('goods/'.$v->goods_id)}}"><img src="{{env('UPLOADS_URL')}}{{$v->goods_goods_img}}" /></a>
      @endforeach
     </div><!--sliderA/-->
     <ul class="pronav">
     ...
     <div class="index-pro1">
      ...
      <div class="index-pro1-list">
       <dl>
        <dt><a href="{{url('goods/'.$v->goods_id)}}"><img src="{{env('UPLOADS_URL')}}{{$v->goods_goods_img}}" /></a></dt>
        <dd class="ip-text"><a href="proinfo.html">{{$v->goods_name}}</a><span>已售：488</span></dd>
        <dd class="ip-price"><strong>¥{{$v->goods_price}}</strong> <span>¥{{$v->goods_marketprice}}</span></dd>
       </dl>
      </div>
      @endforeach
      <div class="clearfix"></div>
